...
$pdf=new FPDF('P','mm','A4');
    ...
    $sql="select 
	concat(_utf8'รหัสสถานี ',`tbl_log_call_center`.`site_id`) AS `site_id`
	,concat(_utf8'สถานีบริการก๊าช NGV ',`tbl_station_ngv`.`site_name`) AS `site_name` 
	from (`tbl_log_call_center`
	join `tbl_station_ngv` on((`tbl_log_call_center`.`site_id` = `tbl_station_ngv`.`site_id`))) 
	where ((`tbl_log_call_center`.`category_type` = _utf8'12') 
	and (`tbl_log_call_center`.`status_call` = _utf8'close'))
	and (`tbl_log_call_center`.`open_call_dte` like _utf8'$dte%')
