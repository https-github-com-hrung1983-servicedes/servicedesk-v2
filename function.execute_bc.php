...
switch ($mode) {
    case "gen_id_bss" :
			$dte = $_REQUEST["dte"];
			mysqli_query($conn,"insert into tbl_job_no (dte) values ('$dte')");
    ...
    if($user_id=="129"){
      ...
      $status_hw = "a";
		...
		,hardware_status='$status_hw',asset_by='$asset_by',sparepartfor='$sparepartfor' where id = '$id_sn'";
			mysqli_query($conn,$sql);
	...
	case "hw.waitrepair" :
		...
		,dte_tme_fix_complete='$dtetme',hardware_status='$status_hw',hardware_no='$serialno',cate_id='$cate_id',hardware_brand='".$hardware_brand."' ,repair_by='$repair_by' $cmd
		...
		,expired_hardware_type_date='$dte_tme_expired_warranty',license_windows_no='$license_windows_no',sparepartfor='$sparepartfor'
		where id = $id_rep";
		echo $sql;
		mysqli_query($conn,$sql);
    ...
    $hardware_status = $_REQUEST["status_hw"];
		$dtetme = getDteTme();
		$sql = "update tbl_hardware_onhand_user set user_id = '$user_expose',dte_tme_form_stock='$dtetme' , hardware_status = '$hardware_status', from_site_id='' where id = '$id_rep'";
		mysqli_query($conn,$sql);
	...
	case "hw.waitrepair.add.serial.form" :
		...
		$dtetme = getDteTme();
		$str = "";
	//	echo getDupplicateSN($cate_id,$serialno);exit;
...
hardware_status,user_enter_system,owner_by,asset_by,status_pm,installation_date,expired_date,hardware_type,warranty_hardware_type_date,expired_hardware_type_date,comment_lot,license_windows_no,sparepartfor)
						values ('','$cate_id','$hardware_brand','$serialno','$dtetme','$dtetme','a',
'$user_enter','$owner_by_sn','$asset_by','n','$dte_tme_install','$dte_tme_expired','$status_deal','$dte_tme_warranty','$dte_tme_expired_warranty','$buy_by','$license_windows_no','$sparepartfor'
)";
			mysqli_query($conn,$sql);
		} else {
	...
	case "count.3g.job" :
		...
		for($i=0;$i<count($site);$i++){
			...
			$site_new1 = explode("T",$site_new);
			$comment = explode("T",$site_new);
     ...
     $from_owner = $_REQUEST["from_owner"];
		 ...
		 if($typer == "add"){
				 if(getSiteDuplicate($site_id)==0){
					$sql = "insert into tbl_site (site_id,site_old_name,site_new_name,site_name,pos,address,province_name,contact,center_phase,from_center,from_owner)
			 	...
			 	$sql = "update tbl_site set site_old_name = '$site_old_name',site_new_name = '$site_new_name',site_name = '$site_name',pos = '$pos',address = '$address',province_name = '$province_name',contact = '$contact',center_phase = '$center_phase',from_center = '$from_center' , from_owner = '$from_owner'
