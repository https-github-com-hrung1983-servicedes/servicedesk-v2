...
$objCSV = fopen("MyXls/sn_onhand_19-10-2016.csv", "r");
...
while (($objArr = fgetcsv($objCSV, 1000, ",")) !== FALSE) {
	...
	$strSQL .="(user_id,cate_id, hardware_brand, hardware_no , hardware_status ,asset_by,owner_by
  ,warranty_hardware_type_date,expired_hardware_type_date,comment_lot ,dte_tme_entry_stock) ";
	$strSQL .="VALUES ";
	$strSQL .="('".$objArr[0]."','".$objArr[1]."','".$objArr[2]."' ";
	$strSQL .=",'".$objArr[3]."','".$objArr[4]."','".$objArr[5]."' ";
  $strSQL .=",'".$objArr[6]."','".$objArr[7]."','".$objArr[8]."','".$objArr[9]."' , NOW() ) ";
...
if($i>0){
	if($c_check["sn_check"] > 0) {
		  echo "Import : ".$objArr[3]." <font color='red'>ERROR was already</font>.</br>";
	}else{
		$objQuery = mysqli_query($conn,$strSQL);
	  echo "Import : ".$objArr[3]." DONE.</br>";
	}
	}
	$i++;
}
