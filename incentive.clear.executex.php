...
if(!checkUser()){    echo Message(35,"red",$titel1,$msg1,"<a href='index.php'> $login </a>");
   ...
   if($typer == ""){                                 
     ...
     //  $status_funds = $_REQUEST["status_funds"];  
       ...
       $ttotal = $_REQUEST["ttotal"]+$bill_total_all; 
       $tot = $_REQUEST["tot"]; 
       $tal = $tot-$ttotal;                                                                     
           if($id != "" && $other_seq ==""){           
              $other_seq = AutoNumber($table." where other_no = $id","other_seq");                                                                                         
              $sql = "Insert into $table (other_no,other_seq,other_dte,bill_description,bill_no,bill_total,bill_total_all)
                       values ('$id','$other_seq','$dte','$bill_description','$bill_no','$bill_total','$bill_total_all')";         
             mysql_query($sql);                                            
           } else if($id != "" && $other_seq !=""){ 
                                
              $sql = "Update $table set other_dte='$dte' ,bill_description='$bill_description',bill_no='$bill_no',bill_total='$bill_total',bill_total_all='$bill_total_all' 
      ...
      mysqli_query($conn,$sql_updatefunds);    
   } else if($typer == "del"){
       $tal = $_REQUEST["ttotal"] - $_REQUEST["bill_total_all"];   
   ...
   //    echo  $_REQUEST["ttotal"] ."<hr>";      
       ...
       mysqli_query($conn,$sql_updatefunds); 
             ...
             $to = $_REQUEST["to"];                
             $sql_check = "update tbl_incentive_ot set status_funds = '$status_funds' , total_funds = '$to',status_check = 's' where id = $id";
