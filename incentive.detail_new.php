...
if(!checkUser()){    echo Message(35,"red",$titel1,$msg1,"<a href='index.php?link=incentive.detail'> $login </a>");
   ...
   if($typer == ""){
       ...
       $distance_result_gps=$_REQUEST["distance_result_gps"];
       $distance_result=$distance_to-$distance_from;

       $datenow = date("Y-m-d");
       if($datenow < "2016-07-30"){
        if($distance_result_gps=="0"){
          $distance_result_agv = (($distance_to-$distance_from)+$distance_result_gps);
        }else{
          $distance_result_agv = (($distance_to-$distance_from)+$distance_result_gps)/2;
        }
       }else{
         $distance_result_agv = (($distance_to-$distance_from)+$distance_result_gps)/2;
       }
       ...
       if($id != "" && $seq_id ==""){
          ...
          $sql = "Insert into $table (id,seq_id,dte,site_id,location_form,location_to,distance_form,distance_to
                                    ,ditstance_result
                                    ,distance_result_gps
                                    ,distance_result_agv
                                    ,express_position,jobdescription,job_no,allowance,incentive,fee_hotel,fee_oil_gas,other_fee)
                   values ('$id','$seq_id','$dte','$site_id','$location_from','$location_to',
                    '$distance_from','$distance_to'
                    ,'$distance_result'
                    ,'$distance_result_gps'
                    ,'$distance_result_agv'
                    ,'$express_','$description','$job_id','$allowan','$incentive','$fee_hotel','$fee_gas','$other_fee')";     //echo $sql;exit;
         mysqli_query($conn,$sql);
          ...
          $sql = "Update $table set dte='$dte',site_id='$site_id',location_form='$location_from',location_to='$location_to',distance_form='$distance_from',
                    ...
                    ,ditstance_result='$distance_result'
                    ,distance_result_gps='$distance_result_gps'
                    ,distance_result_agv='$distance_result_agv'
                    ,express_position='$express_',jobdescription='$description',
...
//echo $sql_del;exit;
    ...
    header("location:incentive.form_new.php?id=".$id."&type=edit");
?>
