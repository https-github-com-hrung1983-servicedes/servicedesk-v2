		...
		<tr>
                ...
                <table width="85%" align="center"  id="table" cellpadding="1" cellspacing="1" border="0">
				   <tr>
					  ...
					  <select name="type_call" id="type_call"  style="width:200pt">
                       <?
						  $sql_customer = "Select customer_id,customer_code,customer_name From tbl_customer where customer_id in ('2','3','4','39')  Order by customer_id";
                     ...
                     <tr>
					  ...
					  <? if($c["type_service"] == ""){ $teamservice="BSS"; }else{ $teamservice=$c["type_service"]; } ?>
                        <select name="cmbServiceType" id="cmbServiceType" style="width:140pt"  />
                           <option value = "MSI" selected>Bizserv Solution Team D</option>
                        </select>
                      </td>
...
onclick="entry_serial_no_bss(txtSid.value,txtJobNo.value,UserEngineerID.value,cmbServiceType.value);">
        ...
        <?// } ?><nobr> HH :
             ...
             ?>
             <select name="open_date_h" id="open_date_h">
               <?for($i=0;$i<24;$i++){?>
             ...
             ?>
             <select name="open_date_t" id="open_date_t">
               <?for($i=0;$i<60;$i++){?>
        ...
        <tr>
         <td height="25" align="left" class="fontBblue" >On Site Date :</td>
         <td height="25" align="left" class="fontBblue"><nobr>
             <?
         ...
         <a href="#" onclick="cdp1.showCalendar(this, 'dteOnSite'); return false;" >
             <img src="image/calendar.png" width="17" height="13" border="0" /></a>
        HH :
             <select name="cmbOnSiteHH" id="cmbOnSiteHH">
               <?for($i=0;$i<24;$i++){?>
                   <option value = "<?=$i?>" <?if($honsite==$i) echo "selected";?>><?=$i?></option>";
               <?}?>
           </select>
              MM :
             <?
             ...
             ?>
             <select name="cmbOnSiteTT" id="cmbOnSiteTT">
               <?for($i=0;$i<60;$i++){?>
                   <option value = "<?=$i?>" <?if($monsite==$i) echo "selected";?>><?=$i?></option>";
               <?}?>
         </select>
         </td>
        </tr>
        ...
        <tr>
         ...
         <a href="#" onclick="cdp1.showCalendar(this, 'dteFixedTime'); return false;" >
             ...
             <select name="cmbFixedTimeHH" id="cmbFixedTimeHH">
               <?for($i=0;$i<24;$i++){?>
                   <option value = "<?=$i?>" <?if($hfix==$i) echo "selected";?>><?=$i?></option>";
               <?}?>
           </select>
             <nobr>MM :
             <?
             ...
             ?>
             <select name="cmbFixedTimeTT" id="cmbFixedTimeTT">
               <?for($i=0;$i<60;$i++){?>
                   <option value = "<?=$i?>" <?if($mfix==$i) echo "selected";?>><?=$i?></option>";
               <?}?>
         </select></td>
        </tr>
        ...
        <tr>
         ...
         <a href="#" onclick="cdp1.showCalendar(this, 'dteClose'); return false;" >
             ...
             ?>
             <select name="cmbCloseHH" id="cmbCloseHH">
               <?for($i=0;$i<24;$i++){?>
                   <option value = "<?=$i?>" <?if($hclose==$i) echo "selected";?>><?=$i?></option>";
               <?}?>
           </select>
             <nobr> MM :
             <?
             $mclose1 = explode(" ",$c["closed_time"]);
             $mclose = getTime($mclose1[0],1);
             ?>
             <select name="cmbCloseTT" id="cmbCloseTT">
               <?for($i=0;$i<60;$i++){?>
                   <option value = "<?=$i?>" <?if($mclose==$i) echo "selected";?>><?=$i?></option>";
               <?}?>
               ?>
           </select></td>
        </tr>
...
<script type="text/javascript">
    ...
    function entry_serial_no_ptt(val,jobno,user_engineer,service_type){
        if(val==""){
           alert('Input Site ID.');
           document.form1.txtSid.focus();
           return;
           ...
           return;
            ...
            properties +="directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=yes, top="+mytop+",left="+myleft;
            window.open("serial_no_all.php?id="+val+"&jobno="+jobno+"&user_engineer="+user_engineer+"&service_type="+service_type,"Entry",properties);
        }
    }
