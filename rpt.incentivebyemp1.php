...
Left Join tbl_incentive_detail ON tbl_incentive_ot.id = tbl_incentive_detail.id";    
		 if($_SESSION["Ustate"] == "admin" || $_SESSION["Ustate"] == "account"){
			...
			// if($user_id != ""){
					 ///$sql .= " Where tbl_incentive_ot.other_receive = '$user_id'";
					 $sql .= " Where tbl_incentive_ot.authen_by = '1'";
			// } else {
			//		$sql .= " Where tbl_incentive_ot.other_receive = '0'";
			//}
		 } else if($_SESSION["Ustate"] != "admin" || $_SESSION["Ustate"] != "account") {    
			  $sql .= " Where tbl_incentive_ot.other_receive = '".$_SESSION["Uid"]."'";  //And tbl_incentive_ot.status_check = 's'
		 }
		$sql .= "  And  status_check = 's'
		And  tbl_incentive_ot.other_date like '$years-$months%'";

		  $sql .= " Group by tbl_incentive_ot.other_no
		  Order by tbl_incentive_ot.other_date , tbl_incentive_ot.id  ";  echo $sql;
            ...
            <form  method="post" name="form1" id="form1" action="#" target="_parent"> 
                ...
                <table width="60%" align="center" class="mytable" id="table7"  cellpadding="1" cellspacing="1">
                    <tr>       					    
                        ...
                        <th class="th" width="30%"><?echo iconv('UTF-8','TIS-620',"รายละเอียดงาน");?></th>        
...
$str_authen = "";
if($c["authen_by"]=="1"){
    $str_authen = "กำลังตรวจสอบบิล \n(กนกพร)"; //1    
}else if($c["authen_by"]=="2"){
     $str_authen = "กำลังตรวจสอบเลขกิโล \n (สันติ)";  //2  
}else if($c["authen_by"]=="3"){
     $str_authen = "ตรวจสอบยอดเงิน \n(กนกพร)";  //3  
}else if($c["authen_by"]=="4"){
     $str_authen = "รอโอนเงิน  \n(กรรณิการ์)";    //4 
     ...
     $str_authen = "ได้รับเงินแล้ว \n ()";    //5
                  ...
                  <tr onmouseover="this.style.backgroundColor='violet';" onmouseout="this.style.backgroundColor='white';" >
                       ...
                       <td style="padding:0px; border-bottom:1px solid #ccc; border-right:1px solid #ccc;border-color:#003366;"  align="right"><?=sprintf("%01.2f",$total_all);?>&nbsp;</td>
